...
const getcategory = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("weong", 500);
    return next(error);
  }
...
const adminLogin = async (req, res, next) => {
  ...
  let token;
  try {
    token = jwt.sign({ login: "Admin" }, process.env.JWT_KEY, {
      expiresIn: "1h",
    });
  } catch (err) {
    const error = new HttpError("not finde", 404);
...
const createitem = async (req, res, next) => {
  const { name, category, praic, Previous_price, Measure, printing, Pants } =
    req.body;
  ...
  const createditem = new Items({
    ...
    Measure,
    printing,
    picture: req.files["image"][0].path,
    picture2: req.files["image1"][0].path,
    Pants,
  });
  try {
